...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiArrowDropDownCircleIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiArrowDropDownCircleIconComponent.type](
      MuiArrowDropDownCircleIconComponent,
      props,
      key)
    }
