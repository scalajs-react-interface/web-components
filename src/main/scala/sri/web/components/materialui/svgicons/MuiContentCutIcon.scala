...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiContentCutIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiContentCutIconComponent.type](
      MuiContentCutIconComponent,
      props,
      key)
    }
