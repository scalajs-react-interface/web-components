...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiLaptopChromebookIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiLaptopChromebookIconComponent.type](
      MuiLaptopChromebookIconComponent,
      props,
      key)
    }
