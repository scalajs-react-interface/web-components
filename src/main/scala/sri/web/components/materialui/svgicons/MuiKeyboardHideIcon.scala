...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiKeyboardHideIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiKeyboardHideIconComponent.type](
      MuiKeyboardHideIconComponent,
      props,
      key)
    }
