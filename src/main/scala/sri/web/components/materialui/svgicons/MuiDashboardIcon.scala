...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiDashboardIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiDashboardIconComponent.type](
      MuiDashboardIconComponent,
      props,
      key)
    }
