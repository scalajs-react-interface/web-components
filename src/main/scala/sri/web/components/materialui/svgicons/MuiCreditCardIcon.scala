...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiCreditCardIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiCreditCardIconComponent.type](
      MuiCreditCardIconComponent,
      props,
      key)
    }
