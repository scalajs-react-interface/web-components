...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiVoicemailIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiVoicemailIconComponent.type](
      MuiVoicemailIconComponent,
      props,
      key)
    }
