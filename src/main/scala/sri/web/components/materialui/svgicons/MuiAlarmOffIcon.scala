...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiAlarmOffIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiAlarmOffIconComponent.type](
      MuiAlarmOffIconComponent,
      props,
      key)
    }
