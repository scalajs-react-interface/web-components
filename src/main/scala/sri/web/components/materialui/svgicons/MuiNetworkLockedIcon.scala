...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiNetworkLockedIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiNetworkLockedIconComponent.type](
      MuiNetworkLockedIconComponent,
      props,
      key)
    }
