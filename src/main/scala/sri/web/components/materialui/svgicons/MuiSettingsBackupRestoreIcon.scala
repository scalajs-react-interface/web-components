...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiSettingsBackupRestoreIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiSettingsBackupRestoreIconComponent.type](
      MuiSettingsBackupRestoreIconComponent,
      props,
      key)
    }
