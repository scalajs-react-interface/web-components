...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiPhotoSizeSelectLargeIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiPhotoSizeSelectLargeIconComponent.type](
      MuiPhotoSizeSelectLargeIconComponent,
      props,
      key)
    }
