...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiErrorOutlineIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiErrorOutlineIconComponent.type](
      MuiErrorOutlineIconComponent,
      props,
      key)
    }
