...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiGamepadIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiGamepadIconComponent.type](
      MuiGamepadIconComponent,
      props,
      key)
    }
