...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiMotorcycleIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiMotorcycleIconComponent.type](
      MuiMotorcycleIconComponent,
      props,
      key)
    }
