...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiPermIdentityIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiPermIdentityIconComponent.type](
      MuiPermIdentityIconComponent,
      props,
      key)
    }
