...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiKeyboardTabIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiKeyboardTabIconComponent.type](
      MuiKeyboardTabIconComponent,
      props,
      key)
    }
