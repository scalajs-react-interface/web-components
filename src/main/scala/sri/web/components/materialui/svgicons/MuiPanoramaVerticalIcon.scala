...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiPanoramaVerticalIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiPanoramaVerticalIconComponent.type](
      MuiPanoramaVerticalIconComponent,
      props,
      key)
    }
