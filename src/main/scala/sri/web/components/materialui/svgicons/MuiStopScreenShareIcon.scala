...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiStopScreenShareIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiStopScreenShareIconComponent.type](
      MuiStopScreenShareIconComponent,
      props,
      key)
    }
