...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiArtTrackIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiArtTrackIconComponent.type](
      MuiArtTrackIconComponent,
      props,
      key)
    }
