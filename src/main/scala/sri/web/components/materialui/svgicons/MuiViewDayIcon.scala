...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiViewDayIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiViewDayIconComponent.type](
      MuiViewDayIconComponent,
      props,
      key)
    }
