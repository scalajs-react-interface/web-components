...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiPermCameraMicIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiPermCameraMicIconComponent.type](
      MuiPermCameraMicIconComponent,
      props,
      key)
    }
