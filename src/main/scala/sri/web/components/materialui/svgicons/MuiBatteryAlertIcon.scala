...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiBatteryAlertIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiBatteryAlertIconComponent.type](
      MuiBatteryAlertIconComponent,
      props,
      key)
    }
