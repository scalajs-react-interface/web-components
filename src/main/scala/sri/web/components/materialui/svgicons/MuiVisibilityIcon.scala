...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiVisibilityIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiVisibilityIconComponent.type](
      MuiVisibilityIconComponent,
      props,
      key)
    }
