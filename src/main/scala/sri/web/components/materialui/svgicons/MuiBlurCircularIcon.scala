...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiBlurCircularIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiBlurCircularIconComponent.type](
      MuiBlurCircularIconComponent,
      props,
      key)
    }
