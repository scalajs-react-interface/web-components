...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiPlayCircleOutlineIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiPlayCircleOutlineIconComponent.type](
      MuiPlayCircleOutlineIconComponent,
      props,
      key)
    }
