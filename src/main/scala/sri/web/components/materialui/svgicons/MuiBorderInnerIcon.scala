...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiBorderInnerIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiBorderInnerIconComponent.type](
      MuiBorderInnerIconComponent,
      props,
      key)
    }
