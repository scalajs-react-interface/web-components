...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiLineWeightIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiLineWeightIconComponent.type](
      MuiLineWeightIconComponent,
      props,
      key)
    }
