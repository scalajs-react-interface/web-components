...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiShowChartIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiShowChartIconComponent.type](
      MuiShowChartIconComponent,
      props,
      key)
    }
