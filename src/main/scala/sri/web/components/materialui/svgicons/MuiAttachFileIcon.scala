...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiAttachFileIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiAttachFileIconComponent.type](
      MuiAttachFileIconComponent,
      props,
      key)
    }
