...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiLeakRemoveIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiLeakRemoveIconComponent.type](
      MuiLeakRemoveIconComponent,
      props,
      key)
    }
