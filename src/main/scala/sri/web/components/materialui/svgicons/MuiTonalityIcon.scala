...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiTonalityIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiTonalityIconComponent.type](
      MuiTonalityIconComponent,
      props,
      key)
    }
