...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiRotate90DegreesCcwIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiRotate90DegreesCcwIconComponent.type](
      MuiRotate90DegreesCcwIconComponent,
      props,
      key)
    }
