...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiViewComfyIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiViewComfyIconComponent.type](
      MuiViewComfyIconComponent,
      props,
      key)
    }
