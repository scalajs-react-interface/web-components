...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiSimCardAlertIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiSimCardAlertIconComponent.type](
      MuiSimCardAlertIconComponent,
      props,
      key)
    }
