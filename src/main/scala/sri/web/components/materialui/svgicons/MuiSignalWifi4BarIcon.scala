...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiSignalWifi4BarIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiSignalWifi4BarIconComponent.type](
      MuiSignalWifi4BarIconComponent,
      props,
      key)
    }
