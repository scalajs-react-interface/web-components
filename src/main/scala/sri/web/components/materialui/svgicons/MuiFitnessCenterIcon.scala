...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiFitnessCenterIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiFitnessCenterIconComponent.type](
      MuiFitnessCenterIconComponent,
      props,
      key)
    }
