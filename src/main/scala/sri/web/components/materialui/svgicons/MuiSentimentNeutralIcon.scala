...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiSentimentNeutralIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiSentimentNeutralIconComponent.type](
      MuiSentimentNeutralIconComponent,
      props,
      key)
    }
