...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiAirplanemodeInactiveIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiAirplanemodeInactiveIconComponent.type](
      MuiAirplanemodeInactiveIconComponent,
      props,
      key)
    }
