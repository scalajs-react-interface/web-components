...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiPhonelinkEraseIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiPhonelinkEraseIconComponent.type](
      MuiPhonelinkEraseIconComponent,
      props,
      key)
    }
