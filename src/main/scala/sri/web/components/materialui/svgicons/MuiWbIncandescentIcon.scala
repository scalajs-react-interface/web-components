...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiWbIncandescentIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiWbIncandescentIconComponent.type](
      MuiWbIncandescentIconComponent,
      props,
      key)
    }
