...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiDirectionsBoatIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiDirectionsBoatIconComponent.type](
      MuiDirectionsBoatIconComponent,
      props,
      key)
    }
