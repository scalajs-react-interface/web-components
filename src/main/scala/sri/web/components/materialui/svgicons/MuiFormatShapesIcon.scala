...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiFormatShapesIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiFormatShapesIconComponent.type](
      MuiFormatShapesIconComponent,
      props,
      key)
    }
