...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiChildFriendlyIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiChildFriendlyIconComponent.type](
      MuiChildFriendlyIconComponent,
      props,
      key)
    }
