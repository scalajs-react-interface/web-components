...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiMultilineChartIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiMultilineChartIconComponent.type](
      MuiMultilineChartIconComponent,
      props,
      key)
    }
