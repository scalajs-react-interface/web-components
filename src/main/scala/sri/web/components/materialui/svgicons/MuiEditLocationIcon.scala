...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiEditLocationIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiEditLocationIconComponent.type](
      MuiEditLocationIconComponent,
      props,
      key)
    }
