...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiSubdirectoryArrowRightIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiSubdirectoryArrowRightIconComponent.type](
      MuiSubdirectoryArrowRightIconComponent,
      props,
      key)
    }
