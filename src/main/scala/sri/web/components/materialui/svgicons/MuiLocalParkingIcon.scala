...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiLocalParkingIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiLocalParkingIconComponent.type](
      MuiLocalParkingIconComponent,
      props,
      key)
    }
