...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiCallReceivedIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiCallReceivedIconComponent.type](
      MuiCallReceivedIconComponent,
      props,
      key)
    }
