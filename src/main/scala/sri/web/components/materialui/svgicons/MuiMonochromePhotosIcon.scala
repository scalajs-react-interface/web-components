...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiMonochromePhotosIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiMonochromePhotosIconComponent.type](
      MuiMonochromePhotosIconComponent,
      props,
      key)
    }
