...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiSwapCallsIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiSwapCallsIconComponent.type](
      MuiSwapCallsIconComponent,
      props,
      key)
    }
