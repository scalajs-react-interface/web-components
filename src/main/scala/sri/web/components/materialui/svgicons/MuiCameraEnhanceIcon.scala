...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiCameraEnhanceIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiCameraEnhanceIconComponent.type](
      MuiCameraEnhanceIconComponent,
      props,
      key)
    }
