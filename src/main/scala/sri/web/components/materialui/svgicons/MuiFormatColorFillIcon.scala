...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiFormatColorFillIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiFormatColorFillIconComponent.type](
      MuiFormatColorFillIconComponent,
      props,
      key)
    }
