...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiChangeHistoryIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiChangeHistoryIconComponent.type](
      MuiChangeHistoryIconComponent,
      props,
      key)
    }
