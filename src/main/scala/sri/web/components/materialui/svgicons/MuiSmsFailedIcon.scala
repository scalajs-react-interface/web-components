...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiSmsFailedIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiSmsFailedIconComponent.type](
      MuiSmsFailedIconComponent,
      props,
      key)
    }
