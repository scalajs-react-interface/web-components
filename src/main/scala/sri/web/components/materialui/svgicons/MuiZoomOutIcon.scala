...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiZoomOutIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiZoomOutIconComponent.type](
      MuiZoomOutIconComponent,
      props,
      key)
    }
