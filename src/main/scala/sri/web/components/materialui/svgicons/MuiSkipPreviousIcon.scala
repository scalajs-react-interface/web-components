...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiSkipPreviousIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiSkipPreviousIconComponent.type](
      MuiSkipPreviousIconComponent,
      props,
      key)
    }
