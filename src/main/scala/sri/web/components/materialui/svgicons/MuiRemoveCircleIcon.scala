...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiRemoveCircleIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiRemoveCircleIconComponent.type](
      MuiRemoveCircleIconComponent,
      props,
      key)
    }
