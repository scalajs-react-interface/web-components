...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiFormatQuoteIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiFormatQuoteIconComponent.type](
      MuiFormatQuoteIconComponent,
      props,
      key)
    }
