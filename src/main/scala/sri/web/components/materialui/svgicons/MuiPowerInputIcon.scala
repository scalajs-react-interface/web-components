...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiPowerInputIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiPowerInputIconComponent.type](
      MuiPowerInputIconComponent,
      props,
      key)
    }
