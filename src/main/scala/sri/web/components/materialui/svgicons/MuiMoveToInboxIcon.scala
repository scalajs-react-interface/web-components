...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiMoveToInboxIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiMoveToInboxIconComponent.type](
      MuiMoveToInboxIconComponent,
      props,
      key)
    }
