...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiCropLandscapeIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiCropLandscapeIconComponent.type](
      MuiCropLandscapeIconComponent,
      props,
      key)
    }
