...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiCloudOffIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiCloudOffIconComponent.type](
      MuiCloudOffIconComponent,
      props,
      key)
    }
