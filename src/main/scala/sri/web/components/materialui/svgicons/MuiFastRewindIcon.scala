...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiFastRewindIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiFastRewindIconComponent.type](
      MuiFastRewindIconComponent,
      props,
      key)
    }
