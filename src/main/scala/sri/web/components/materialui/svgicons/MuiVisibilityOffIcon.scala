...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiVisibilityOffIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiVisibilityOffIconComponent.type](
      MuiVisibilityOffIconComponent,
      props,
      key)
    }
