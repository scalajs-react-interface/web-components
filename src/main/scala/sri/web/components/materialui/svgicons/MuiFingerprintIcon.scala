...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiFingerprintIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiFingerprintIconComponent.type](
      MuiFingerprintIconComponent,
      props,
      key)
    }
