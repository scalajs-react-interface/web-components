...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiExitToAppIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiExitToAppIconComponent.type](
      MuiExitToAppIconComponent,
      props,
      key)
    }
