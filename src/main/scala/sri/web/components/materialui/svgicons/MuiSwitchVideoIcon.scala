...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiSwitchVideoIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiSwitchVideoIconComponent.type](
      MuiSwitchVideoIconComponent,
      props,
      key)
    }
