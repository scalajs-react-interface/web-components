...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiLayersClearIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiLayersClearIconComponent.type](
      MuiLayersClearIconComponent,
      props,
      key)
    }
