...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiBrightness4IconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiBrightness4IconComponent.type](
      MuiBrightness4IconComponent,
      props,
      key)
    }
