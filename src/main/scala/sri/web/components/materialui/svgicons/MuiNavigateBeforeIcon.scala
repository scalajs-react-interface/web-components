...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiNavigateBeforeIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiNavigateBeforeIconComponent.type](
      MuiNavigateBeforeIconComponent,
      props,
      key)
    }
