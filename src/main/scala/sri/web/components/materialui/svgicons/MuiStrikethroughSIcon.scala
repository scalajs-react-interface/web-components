...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiStrikethroughSIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiStrikethroughSIconComponent.type](
      MuiStrikethroughSIconComponent,
      props,
      key)
    }
