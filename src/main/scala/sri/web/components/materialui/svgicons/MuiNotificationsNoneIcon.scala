...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiNotificationsNoneIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiNotificationsNoneIconComponent.type](
      MuiNotificationsNoneIconComponent,
      props,
      key)
    }
