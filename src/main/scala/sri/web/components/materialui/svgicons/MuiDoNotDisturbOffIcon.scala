...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiDoNotDisturbOffIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiDoNotDisturbOffIconComponent.type](
      MuiDoNotDisturbOffIconComponent,
      props,
      key)
    }
