...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiBookmarkIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiBookmarkIconComponent.type](
      MuiBookmarkIconComponent,
      props,
      key)
    }
