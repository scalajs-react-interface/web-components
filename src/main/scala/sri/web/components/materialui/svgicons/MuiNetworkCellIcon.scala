...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiNetworkCellIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiNetworkCellIconComponent.type](
      MuiNetworkCellIconComponent,
      props,
      key)
    }
