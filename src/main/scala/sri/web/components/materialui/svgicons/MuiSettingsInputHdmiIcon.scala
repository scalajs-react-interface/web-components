...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiSettingsInputHdmiIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiSettingsInputHdmiIconComponent.type](
      MuiSettingsInputHdmiIconComponent,
      props,
      key)
    }
