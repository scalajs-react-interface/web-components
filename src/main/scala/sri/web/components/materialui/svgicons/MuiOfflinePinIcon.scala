...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiOfflinePinIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiOfflinePinIconComponent.type](
      MuiOfflinePinIconComponent,
      props,
      key)
    }
