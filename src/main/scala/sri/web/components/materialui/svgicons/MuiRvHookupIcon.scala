...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiRvHookupIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiRvHookupIconComponent.type](
      MuiRvHookupIconComponent,
      props,
      key)
    }
