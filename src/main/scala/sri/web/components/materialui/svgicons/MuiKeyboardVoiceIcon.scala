...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiKeyboardVoiceIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiKeyboardVoiceIconComponent.type](
      MuiKeyboardVoiceIconComponent,
      props,
      key)
    }
