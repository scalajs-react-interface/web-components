...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiFilter2IconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiFilter2IconComponent.type](
      MuiFilter2IconComponent,
      props,
      key)
    }
