...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiPauseCircleFilledIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiPauseCircleFilledIconComponent.type](
      MuiPauseCircleFilledIconComponent,
      props,
      key)
    }
