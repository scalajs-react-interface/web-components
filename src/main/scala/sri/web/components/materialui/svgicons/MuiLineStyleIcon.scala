...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiLineStyleIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiLineStyleIconComponent.type](
      MuiLineStyleIconComponent,
      props,
      key)
    }
