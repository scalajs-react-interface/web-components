...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiBluetoothConnectedIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiBluetoothConnectedIconComponent.type](
      MuiBluetoothConnectedIconComponent,
      props,
      key)
    }
