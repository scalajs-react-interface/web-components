...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiChevronLeftIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiChevronLeftIconComponent.type](
      MuiChevronLeftIconComponent,
      props,
      key)
    }
