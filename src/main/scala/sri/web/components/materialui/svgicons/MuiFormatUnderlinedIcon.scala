...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiFormatUnderlinedIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiFormatUnderlinedIconComponent.type](
      MuiFormatUnderlinedIconComponent,
      props,
      key)
    }
