...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiUnarchiveIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiUnarchiveIconComponent.type](
      MuiUnarchiveIconComponent,
      props,
      key)
    }
