...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiExplicitIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiExplicitIconComponent.type](
      MuiExplicitIconComponent,
      props,
      key)
    }
