...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiArrowDownwardIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiArrowDownwardIconComponent.type](
      MuiArrowDownwardIconComponent,
      props,
      key)
    }
