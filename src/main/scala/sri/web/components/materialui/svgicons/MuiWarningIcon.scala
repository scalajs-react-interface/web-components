...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiWarningIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiWarningIconComponent.type](
      MuiWarningIconComponent,
      props,
      key)
    }
