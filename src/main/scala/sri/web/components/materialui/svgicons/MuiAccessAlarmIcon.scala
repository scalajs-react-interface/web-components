...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiAccessAlarmIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiAccessAlarmIconComponent.type](
      MuiAccessAlarmIconComponent,
      props,
      key)
    }
