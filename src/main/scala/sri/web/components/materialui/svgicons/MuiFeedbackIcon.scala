...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiFeedbackIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiFeedbackIconComponent.type](
      MuiFeedbackIconComponent,
      props,
      key)
    }
