...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiCheckBoxIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiCheckBoxIconComponent.type](
      MuiCheckBoxIconComponent,
      props,
      key)
    }
