...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiAirlineSeatReclineNormalIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiAirlineSeatReclineNormalIconComponent.type](
      MuiAirlineSeatReclineNormalIconComponent,
      props,
      key)
    }
