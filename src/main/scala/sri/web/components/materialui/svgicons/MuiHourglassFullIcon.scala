...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiHourglassFullIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiHourglassFullIconComponent.type](
      MuiHourglassFullIconComponent,
      props,
      key)
    }
