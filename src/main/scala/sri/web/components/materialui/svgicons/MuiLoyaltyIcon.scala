...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiLoyaltyIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiLoyaltyIconComponent.type](
      MuiLoyaltyIconComponent,
      props,
      key)
    }
