...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiRotateLeftIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiRotateLeftIconComponent.type](
      MuiRotateLeftIconComponent,
      props,
      key)
    }
