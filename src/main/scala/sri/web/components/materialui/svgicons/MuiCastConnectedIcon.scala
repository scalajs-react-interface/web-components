...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiCastConnectedIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiCastConnectedIconComponent.type](
      MuiCastConnectedIconComponent,
      props,
      key)
    }
