...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiFormatLineSpacingIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiFormatLineSpacingIconComponent.type](
      MuiFormatLineSpacingIconComponent,
      props,
      key)
    }
