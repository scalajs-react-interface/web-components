...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiQueryBuilderIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiQueryBuilderIconComponent.type](
      MuiQueryBuilderIconComponent,
      props,
      key)
    }
