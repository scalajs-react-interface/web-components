...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiLockOutlineIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiLockOutlineIconComponent.type](
      MuiLockOutlineIconComponent,
      props,
      key)
    }
