...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiLastPageIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiLastPageIconComponent.type](
      MuiLastPageIconComponent,
      props,
      key)
    }
