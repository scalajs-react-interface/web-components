...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiKeyboardCapslockIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiKeyboardCapslockIconComponent.type](
      MuiKeyboardCapslockIconComponent,
      props,
      key)
    }
