...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiFlightTakeoffIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiFlightTakeoffIconComponent.type](
      MuiFlightTakeoffIconComponent,
      props,
      key)
    }
