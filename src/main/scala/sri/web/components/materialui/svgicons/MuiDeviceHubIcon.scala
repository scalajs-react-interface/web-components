...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiDeviceHubIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiDeviceHubIconComponent.type](
      MuiDeviceHubIconComponent,
      props,
      key)
    }
