...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiSettingsInputComponentIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiSettingsInputComponentIconComponent.type](
      MuiSettingsInputComponentIconComponent,
      props,
      key)
    }
