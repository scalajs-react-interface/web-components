...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiTrendingUpIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiTrendingUpIconComponent.type](
      MuiTrendingUpIconComponent,
      props,
      key)
    }
