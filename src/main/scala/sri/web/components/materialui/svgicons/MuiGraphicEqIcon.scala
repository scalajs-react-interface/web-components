...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiGraphicEqIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiGraphicEqIconComponent.type](
      MuiGraphicEqIconComponent,
      props,
      key)
    }
