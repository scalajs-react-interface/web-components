...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiWeekendIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiWeekendIconComponent.type](
      MuiWeekendIconComponent,
      props,
      key)
    }
