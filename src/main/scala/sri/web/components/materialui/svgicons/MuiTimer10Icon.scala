...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiTimer10IconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiTimer10IconComponent.type](
      MuiTimer10IconComponent,
      props,
      key)
    }
