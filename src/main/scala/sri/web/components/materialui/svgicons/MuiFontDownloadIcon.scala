...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiFontDownloadIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiFontDownloadIconComponent.type](
      MuiFontDownloadIconComponent,
      props,
      key)
    }
