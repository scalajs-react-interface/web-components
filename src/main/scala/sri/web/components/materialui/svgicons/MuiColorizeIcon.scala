...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiColorizeIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiColorizeIconComponent.type](
      MuiColorizeIconComponent,
      props,
      key)
    }
