...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiPermMediaIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiPermMediaIconComponent.type](
      MuiPermMediaIconComponent,
      props,
      key)
    }
