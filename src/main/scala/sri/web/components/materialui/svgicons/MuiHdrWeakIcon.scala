...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiHdrWeakIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiHdrWeakIconComponent.type](
      MuiHdrWeakIconComponent,
      props,
      key)
    }
