...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiTapAndPlayIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiTapAndPlayIconComponent.type](
      MuiTapAndPlayIconComponent,
      props,
      key)
    }
