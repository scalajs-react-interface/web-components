...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiDesktopMacIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiDesktopMacIconComponent.type](
      MuiDesktopMacIconComponent,
      props,
      key)
    }
