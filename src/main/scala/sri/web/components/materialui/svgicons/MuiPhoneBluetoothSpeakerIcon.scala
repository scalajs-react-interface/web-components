...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiPhoneBluetoothSpeakerIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiPhoneBluetoothSpeakerIconComponent.type](
      MuiPhoneBluetoothSpeakerIconComponent,
      props,
      key)
    }
