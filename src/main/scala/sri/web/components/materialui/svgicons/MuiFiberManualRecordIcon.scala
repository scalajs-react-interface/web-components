...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiFiberManualRecordIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiFiberManualRecordIconComponent.type](
      MuiFiberManualRecordIconComponent,
      props,
      key)
    }
