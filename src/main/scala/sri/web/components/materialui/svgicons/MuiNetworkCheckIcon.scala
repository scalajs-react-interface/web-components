...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiNetworkCheckIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiNetworkCheckIconComponent.type](
      MuiNetworkCheckIconComponent,
      props,
      key)
    }
