...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiBookmarkBorderIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiBookmarkBorderIconComponent.type](
      MuiBookmarkBorderIconComponent,
      props,
      key)
    }
