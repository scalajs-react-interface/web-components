...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiControlPointIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiControlPointIconComponent.type](
      MuiControlPointIconComponent,
      props,
      key)
    }
