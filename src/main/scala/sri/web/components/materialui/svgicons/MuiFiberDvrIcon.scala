...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiFiberDvrIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiFiberDvrIconComponent.type](
      MuiFiberDvrIconComponent,
      props,
      key)
    }
