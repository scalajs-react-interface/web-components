...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiPlaylistPlayIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiPlaylistPlayIconComponent.type](
      MuiPlaylistPlayIconComponent,
      props,
      key)
    }
