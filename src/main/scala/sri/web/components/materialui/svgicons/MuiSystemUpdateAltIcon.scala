...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiSystemUpdateAltIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiSystemUpdateAltIconComponent.type](
      MuiSystemUpdateAltIconComponent,
      props,
      key)
    }
