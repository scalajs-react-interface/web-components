...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiPartyModeIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiPartyModeIconComponent.type](
      MuiPartyModeIconComponent,
      props,
      key)
    }
