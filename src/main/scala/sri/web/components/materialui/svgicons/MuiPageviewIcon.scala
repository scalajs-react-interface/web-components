...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiPageviewIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiPageviewIconComponent.type](
      MuiPageviewIconComponent,
      props,
      key)
    }
