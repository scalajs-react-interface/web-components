...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiRoundedCornerIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiRoundedCornerIconComponent.type](
      MuiRoundedCornerIconComponent,
      props,
      key)
    }
