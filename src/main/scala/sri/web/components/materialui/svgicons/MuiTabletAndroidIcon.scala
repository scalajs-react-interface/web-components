...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiTabletAndroidIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiTabletAndroidIconComponent.type](
      MuiTabletAndroidIconComponent,
      props,
      key)
    }
