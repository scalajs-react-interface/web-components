...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiReorderIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiReorderIconComponent.type](
      MuiReorderIconComponent,
      props,
      key)
    }
