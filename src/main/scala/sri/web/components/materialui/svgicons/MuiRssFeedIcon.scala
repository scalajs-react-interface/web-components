...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiRssFeedIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiRssFeedIconComponent.type](
      MuiRssFeedIconComponent,
      props,
      key)
    }
