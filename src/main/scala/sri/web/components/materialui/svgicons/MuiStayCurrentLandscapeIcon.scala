...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiStayCurrentLandscapeIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiStayCurrentLandscapeIconComponent.type](
      MuiStayCurrentLandscapeIconComponent,
      props,
      key)
    }
