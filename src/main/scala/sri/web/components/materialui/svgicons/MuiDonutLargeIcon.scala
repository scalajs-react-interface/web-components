...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiDonutLargeIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiDonutLargeIconComponent.type](
      MuiDonutLargeIconComponent,
      props,
      key)
    }
