...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiMarkunreadMailboxIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiMarkunreadMailboxIconComponent.type](
      MuiMarkunreadMailboxIconComponent,
      props,
      key)
    }
