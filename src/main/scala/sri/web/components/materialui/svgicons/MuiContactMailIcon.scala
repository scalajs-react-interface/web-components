...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiContactMailIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiContactMailIconComponent.type](
      MuiContactMailIconComponent,
      props,
      key)
    }
