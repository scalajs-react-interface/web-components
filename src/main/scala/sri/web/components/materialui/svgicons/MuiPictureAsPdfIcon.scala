...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiPictureAsPdfIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiPictureAsPdfIconComponent.type](
      MuiPictureAsPdfIconComponent,
      props,
      key)
    }
