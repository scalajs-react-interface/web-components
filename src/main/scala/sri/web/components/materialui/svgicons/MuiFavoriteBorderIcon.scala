...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiFavoriteBorderIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiFavoriteBorderIconComponent.type](
      MuiFavoriteBorderIconComponent,
      props,
      key)
    }
