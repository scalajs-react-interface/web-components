...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiLockOpenIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiLockOpenIconComponent.type](
      MuiLockOpenIconComponent,
      props,
      key)
    }
