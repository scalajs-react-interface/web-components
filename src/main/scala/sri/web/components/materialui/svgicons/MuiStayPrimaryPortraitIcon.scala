...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiStayPrimaryPortraitIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiStayPrimaryPortraitIconComponent.type](
      MuiStayPrimaryPortraitIconComponent,
      props,
      key)
    }
