...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiBlurLinearIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiBlurLinearIconComponent.type](
      MuiBlurLinearIconComponent,
      props,
      key)
    }
