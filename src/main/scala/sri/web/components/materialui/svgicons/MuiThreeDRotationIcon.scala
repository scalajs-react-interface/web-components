...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiThreeDRotationIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiThreeDRotationIconComponent.type](
      MuiThreeDRotationIconComponent,
      props,
      key)
    }
