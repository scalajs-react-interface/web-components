...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiHealingIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiHealingIconComponent.type](
      MuiHealingIconComponent,
      props,
      key)
    }
