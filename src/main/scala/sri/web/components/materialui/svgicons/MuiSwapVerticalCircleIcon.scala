...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiSwapVerticalCircleIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiSwapVerticalCircleIconComponent.type](
      MuiSwapVerticalCircleIconComponent,
      props,
      key)
    }
