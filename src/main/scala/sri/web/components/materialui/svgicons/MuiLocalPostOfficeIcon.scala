...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiLocalPostOfficeIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiLocalPostOfficeIconComponent.type](
      MuiLocalPostOfficeIconComponent,
      props,
      key)
    }
