...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiGTranslateIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiGTranslateIconComponent.type](
      MuiGTranslateIconComponent,
      props,
      key)
    }
