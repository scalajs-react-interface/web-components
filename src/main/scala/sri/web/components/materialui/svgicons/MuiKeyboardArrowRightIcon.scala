...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiKeyboardArrowRightIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiKeyboardArrowRightIconComponent.type](
      MuiKeyboardArrowRightIconComponent,
      props,
      key)
    }
