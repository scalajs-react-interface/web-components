...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiPhotoSizeSelectActualIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiPhotoSizeSelectActualIconComponent.type](
      MuiPhotoSizeSelectActualIconComponent,
      props,
      key)
    }
