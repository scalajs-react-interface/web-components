...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiFormatIndentIncreaseIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiFormatIndentIncreaseIconComponent.type](
      MuiFormatIndentIncreaseIconComponent,
      props,
      key)
    }
