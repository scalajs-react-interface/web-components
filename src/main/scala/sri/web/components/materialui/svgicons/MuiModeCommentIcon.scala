...
@exclude key: String | Int = null ): ReactElement { type Instance = MuiModeCommentIconComponent.type}  = {
      import DangerousUnionToJSAnyImplicit._
      val props = FunctionObjectMacro()
      extraProps.foreach(v => { MergeJSObjects(props, v) })
    CreateElementJS[MuiModeCommentIconComponent.type](
      MuiModeCommentIconComponent,
      props,
      key)
    }
